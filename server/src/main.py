        ...
        try:
            ...
            if packet.shared_action == proto_model.ClientSA.DISPATCH_WORKER:
                ...
                proto_model.write_packet(proto_model.ServerPacket(worker_id=None, shared_action=proto_model.ServerSA.UUID_AVAILABLE)
                                 .add_data("uuid", _uuid))
                _worker: ic_worker.ICWorker = WORKER_VERSION_MAP[shared_constants.WorkerType[packet.data["worker_type"]]][version](_uuid, _ic, None)
                # Add to active group
                ...
                _worker = get_worker_by_uuid(packet.worker_id, pop=True)
                if (_worker == None):
                    continue
                try:
                    _worker.shutdown()
                    ...
                    util.log(util.LogLevel.WARNING, "Error while shutting down worker: ", error_trace=True)
...
def get_worker_by_uuid(uuid: str, pop: bool=False):
    if uuid == None:
        return None
    if ACTIVE_WORKERS.__contains__(uuid):
        if pop:
            return ACTIVE_WORKERS.pop(uuid)
